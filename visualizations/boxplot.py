...
df = pd.read_csv('../data/population_1960_2017_GOEDE.csv' ,header=0, sep=',', error_bad_lines=False, encoding = 'latin-1')
...
for country in countries:
    vals = df[df['Country_Name'] == country]
    vals = vals.values.tolist()
    vals = vals[0]
    vals = vals[1:]
    list_country = [country for i in range(len(vals))]
    yy = yy + vals
    g = g + list_country
